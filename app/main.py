...
async def login_page(request: Request):
    ...
    if val:
        ...
        if ses:
            user = await get_user_by_id(ses["user_id"])
            if user:
                return HTMLResponse(read_html("app/public/home.html"))
    return HTMLResponse(read_html("app/public/index.html"))
            ...
            if user:
                try:
                    while True:
                        db = get_db_connection()
                        cursor = db.cursor(dictionary=True)
                        cursor.execute(f'SELECT macaddr, value, timestamp FROM temperature WHERE macaddr IN (SELECT name FROM devices WHERE user_id={user})')
